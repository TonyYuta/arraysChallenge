 ...
 * In that case, an array contains only one duplicate but what if it contains multiple duplicates?
...
public class FindDuplicates {
	...
	public static void main(String[] args) {
		...
		System.out.println((dup.duplicatesInt(arr02).size() > 0) ? "FindDuplicates in arr02: " + dup.duplicatesInt(arr02) : "There are no duplicates in arr02");
		System.out.println((dup.duplicatesInt(arr03).size() > 0) ? "FindDuplicates in arr03: " + dup.duplicatesInt(arr03) : "There are no duplicates in arr03");
	}
	...
	public Object[] duplicatesIntArray(int[] arr) {
		ts = new TreeSet<>();
		for(int i = 0; i < arr.length-1; i++) {
			for (int j = i+1; j < arr.length; j++) {
				if (arr[i] == arr[j]) {
